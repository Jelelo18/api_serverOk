...
module.exports =
    class Repository {
        ...
        sortName() {
            return collectionManager.sortName(this.objectsList);
        }
        sortCategory() {
            return collectionManager.sortCategory(this.objectsList);
        }
        nameStartsWith(name) {
            return collectionManager.nameStartsWith(this.objectsList, name);
        }
        searchByName(name) {
            return collectionManager.searchByName(this.objectsList, name);
        }
        searchByCategory(category) {
            return collectionManager.searchByCategory(this.objectsList, category);
        }
        process(params){
            console.log(collectionManager.process(this.objectsList, params));
            console.log("in repo");
            return collectionManager.process(this.objectsList, params);
        }
        remove(id) {
            let index = 0;
            for (let object of this.objectsList) {
